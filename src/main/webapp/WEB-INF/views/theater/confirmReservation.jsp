      ...
      <c:choose>
        <c:when test="${not empty reservation}">
          <div class="r-text" style="line-height: 2;">
            <p><strong class="r-strong">🎫 예매 번호:</strong> ${reservation.reservationID}</p>
            <p><strong class="r-strong">👤 회원 이름:</strong> ${reservation.mem_Name}</p>
            <p><strong class="r-strong">🎬 영화 제목:</strong> ${reservation.movieType}</p>
            <p><strong class="r-strong">🏛 상영관:</strong> ${reservation.auditoriumName}</p>
            <p><strong class="r-strong">🕒 상영 일시:</strong> ${reservation.screeningDate}</p>
            <p><strong class="r-strong">💺 좌석 번호:</strong> ${reservation.seatName}</p>
          </div>
        </c:when>
        <c:otherwise>
